[T4Scaffolding.Scaffolder(Description = "Enter a description of AjaxPartialViewModel here")][CmdletBinding()]
param(      
...
if (!$ModelType) {
	if ($ViewModelName.EndsWith("ViewModel", [StringComparison]::OrdinalIgnoreCase)) {
		...
		if ($foundModelType) {
			$ViewModelName = [string](Get-PluralizedWord $foundModelType.Name) + "PartialViewModel"
		}
	}
	...
	if (!$ViewModelName.EndsWith("PartialViewModel", [StringComparison]::OrdinalIgnoreCase)) {
		$ViewModelName = $ViewModelName + "PartialViewModel"
	}
}
...
Add-ProjectItemViaTemplate $outputPath -Template AjaxPartialViewModel `
	-Model @{ 
